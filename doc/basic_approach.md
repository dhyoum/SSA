...
1.2) 간선의 가중치에 음수가 없다.
1.2.A) 간선의 가중치가 모두 동일한가?( ex. 1)
	BFS를 이용한다.
	1.2.A.a) 단순한 BFS 이용한다.
	1.2.A.b) 단순한 BFS로 부족하면 즉, 탐색의 깊이가 너무 깊거나
			메모리의 양이 부족할 때 양방향 BFS를 이용한다.

	1.2.A.c) 만약 양방향BFS에서 역방향으로 움직이기가 어렵다면 점점 깊어지는 탐색을 사용한다.

1.2.B) 간선의 가중치가 다르다.
1.2.B.a) 다익스트라 알고리즘을 이용한다.
